...
class Qwen3_5ForCausalLMMTP(nn.Module):
    ...
    def __init__(
        ...
        # GPTQ-Int4 checkpoints (e.g. palmfuture/Qwen3.8-27B-GPTQ-Int4) keep
        # `mtp.*` weights in their original high precision: the GPTQModel
        # dynamic dict lists them as a negative (`-:.*\\.mtp\\..*`) match, so
        # they are excluded from quantization. Without this, SGLang decodes
        # the bf16 MTP head through GPTQ/Marlin kernels -> garbage drafts ->
        # accept rate ~0. (Mirrors the existing Quark/ModelOpt branches.)
